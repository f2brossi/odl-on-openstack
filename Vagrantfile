...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  neutron_ex_ip = "192.168.111.20"
  compute_ex_ip_base = "192.168.111."
  compute_ips = num_compute_nodes.times.collect { |n| compute_ip_base + "#{n+21}" }
  compute_ex_ips = num_compute_nodes.times.collect { |n| compute_ex_ip_base + "#{n+21}" }
  ...
  config.vm.define "devstack-control", primary: true do |control|
    ...
    ## control.vm.network "private_network", ip: "#{neutron_ex_ip}", virtualbox__intnet: "mylocalnet", auto_config: true
  ...
  num_compute_nodes.times do |n|
    config.vm.define "devstack-compute-#{n+1}", autostart: true do |compute|
      ...
      ## compute.vm.network "private_network", ip: "#{compute_ex_ip}", virtualbox__intnet: "mylocalnet", auto_config: true
